...
struct dbg_dereference_check {
   char *start;
   const RB &result;
   dbg_dereference_check(char *start, const RB &result): start(start), result(result) { }
   char dereference_p(char *p) {
      if( !(p < start+result.Size()) ) {
         p = 0;
      }
      return *p;
   }
...
void ServiceImpl::GetUsers(std::vector<IBPP::User>& users)
{
	...
	users.clear();
	char* p = result.Self();
   dbg_dereference_check chk(p,result);
	if (*p != isc_info_svc_get_users)
		throw SQLExceptionImpl(status, "Service::GetUsers", _("isc_service_query returned unexpected answer"));

	p += 3;	// Skips the 'isc_info_svc_get_users' and its total length
	IBPP::User user;
	while (chk.dereference_p(p) != isc_info_end)
	{
		if (chk.dereference_p(p) == isc_spb_sec_userid)
		{
			user.userid = (uint32_t)(*gds.Call()->m_vax_integer)(p+1, 4);
			p += 5;
		}
		else if (chk.dereference_p(p) == isc_spb_sec_groupid)
		{
			user.groupid = (uint32_t)(*gds.Call()->m_vax_integer)(p+1, 4);
			p += 5;
		}
